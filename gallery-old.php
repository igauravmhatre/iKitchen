...
<div class="gallery pages">
	  <div class="container">
		...
		if (isset($_GET['add'])){
			...
			$query = "INSERT INTO favorite(user_id,content_id) VALUES('" . $user_id . "', '" . $id . "')";
			$fire2 = mysqli_query($con,$query) or die("Cannot add data from database".mysqli_error($con));
		}
		 if($sub==1){
		 	//$id=$_SESSION['usr_id'];
				...
				<div class="gallery-1">
					...
					<form role="form" action = "<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="delete">
						...
						<div class="row looping">
								...
								<div class="col-md-9">
								<?php
								$query1 = "SELECT * FROM favorite where content_id = $row3[content_id]";
								$fire4 = mysqli_query($con,$query1);
								
									if(!$row4 = mysqli_fetch_assoc($fire4)){
										?>
										<a class="btn btn-sm btn-danger" href="<?php $_SERVER['PHP_SELF']?>?add=<?php echo $row3['content_id']?>">add to fav</a>
										<?php
									}
									else{
										?>
